...
var standard_client_deposit_files = document.getElementById(
  "standard_client_deposit_files"
);
...
//Tablesorter
var table = document.getElementById("deposit-tablesorter");
  ...
  if (index != number_of_files) {
    ...
    if (validateExtension(file_extension)) {
      ...
      reader.onload = function (e) {
        if (getFileData(e) != 0) {
          setTimeout(
            agregaFilaTablaDeposito.bind(null, total_uploaded_files),
            1
          );
...
$("#modalShowData").modal({
  ready: function (modal, trigger) {
    ...
    $("#modalShowData .modal-content").append(
      "<h5>Datos de CFDI</h5>" +
        '<span class="client-name-modal valign-wrapper" data-client-name="' +
        trigger.attr("data-client-name") +
        '" data-client-rfc="' +
        trigger.attr("data-client-rfc") +
        '"><b>Cliente: </b>' +
        trigger.attr("data-client-name") +
        "</span><b>RFC: </b>" +
        trigger.attr("data-client-rfc") +
    ...
    if (trigger.hasClass("red-text")) {
      $(".client-name-modal").append(
        '&nbsp;&nbsp;<a href="#newClientModal" class="modal-trigger newClientFromPolicy" onclick="closeDataModal();">' +
...
function agregaFilaTablaDeposito(index) {
  $("tbody").append(
    '<tr data-file-index="' +
      index +
      '" data-rfc-client="' +
      jsonFilesData[index].receptor.rfcReceptor +
      '">' +
      ...
      index +
      '">' +
      '<span class="truncate client-name selectable modal-trigger" href="#modalShowData" style="width: 90%;" data-client-name="' +
      jsonFilesData[index].receptor.nombreReceptor +
      '" data-client-rfc="' +
      jsonFilesData[index].receptor.rfcReceptor +
      '" data-serie="' +
...
function setConceptsToJson(json_index) {
  ...
  $(
    "#deposit-tablesorter #conceptList" +
      json_index +
  ...
  if (amount_undefined_concepts == 0) {
    if (
      $(
        "#deposit-tablesorter tbody tr:nth-child(" +
          (json_index + 1) +
          ") td:last"
      ).children("i").length == 1
    ) {
...
function verifyClient(row_index) {
  ...
  $.ajax({
    ...
    data: {
      _token: CSRF_TOKEN,
      handler: "getClient",
      client_rfc: jsonFilesData[row_index].receptor.rfcReceptor,
    },
  })
    .done(function (data) {
      if (data.length === 0) {
        ...
        $(
          'tr[data-file-index="' +
            row_index +
            '"] td:nth-child(7) .newClientFromPolicy'
        ).remove();
...
function setClientAccountToJson(row_index, client_accounting_account) {
  $.extend(
    jsonFilesData[row_index].cliente.cuentaContable,
    client_accounting_account
  );
}
...
function sendJsonFiles() {
  ...
  if (verifyUnregisteredClients() != 0) {
    Materialize.toast(
      "No se pudo procesar la petición. El cliente de algún comprobante no está registrado.",
      4000
    );
  } else {
    if (verifyUndefinedDates() != 0) {
      ...
      $("tbody tr").each(function (index) {
        ...
        if (
          ...
          jsonFilesData[indexJsonFile].datosDestino.bancoDestino = $(
            ".destiny-" + indexJsonFile + " select option:selected"
          ).val();
          jsonFilesData[indexJsonFile].datosDestino.cuentaBancariaDestino = $(
            "#bank-account-destiny-" + indexJsonFile
          ).val();
      ...
      $.ajax({
        ...
        data: {
          ...
          jsonFiles: jsonFiles,
          generateByClient: generate_by_toggle,
