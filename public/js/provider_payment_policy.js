...
var standard_provider_payment_files = document.getElementById(
  "standard_provider_payment_files"
);
var honorarium_provider_payment_files = document.getElementById(
  "honorarium_provider_payment_files"
);
  ...
  if (index != number_of_files) {
    ...
    if (validateExtension(file_extension)) {
      ...
      reader.onload = function (e) {
        getFileData(e);
        setTimeout(
          agregaFilaTablaProvision.bind(null, total_uploaded_files),
          1
        );
    ...
    if (unreaded_files.length > 0) {
      ...
      Materialize.toast(
        "Ocurrió un error al procesar los siguientes archivos: " +
          unreaded_files,
        7000
      );
      unreaded_files = [];
...
function obtenerDatosXML(xml) {
  ...
  if (comprobante.attr("version") == "3.2") {
    ...
    datosXML.receptor.nombreReceptor = validaDatoDefinido(
      datosXML.receptor.nombreReceptor
    );
...
$("#modalShowData").modal({
  ready: function (modal, trigger) {
    ...
    $("#modalShowData .modal-content").append(
      "<h5>Datos de CFDI</h5>" +
        '<span class="provider-name-modal valign-wrapper" data-provider-name="' +
        trigger.attr("data-provider-name") +
        '" data-provider-rfc="' +
        trigger.attr("data-provider-rfc") +
        '"><b>Proveedor: </b>' +
        trigger.attr("data-provider-name") +
        "</span><b>RFC: </b>" +
        trigger.attr("data-provider-rfc") +
        "<br><b>Serie: </b>" +
        trigger.attr("data-serie")
    );
    if (trigger.hasClass("red-text")) {
      $(".provider-name-modal").append(
        '&nbsp;&nbsp;<a href="#newProviderModal" class="modal-trigger newProviderFromPolicy" onclick="closeDataModal();">' +
...
function agregaFilaTablaProvision(index) {
  $("tbody").append(
    '<tr data-file-index="' +
      index +
      '" data-rfc-provider="' +
      jsonFilesData[index].emisor.rfcEmisor +
      '">' +
      '<td class="center-align">' +
      '<input type="checkbox" class="filled-in row-select" id="row-select-' +
      index +
      '"/>' +
      '<label for="row-select-' +
      index +
      ...
      '<input id="fecha-' +
      index +
      '" class="date" type="date" max="' +
      setTodayDate() +
      '" onchange="setDateValue(this);">' +
      "</td>" +
      '<td class="center-align" style="width: 10%;">' +
      ...
      index +
      '">' +
      '<span class="truncate provider-name selectable modal-trigger" href="#modalShowData" style="width: 90%;" data-provider-name="' +
      jsonFilesData[index].emisor.nombreEmisor +
      '" data-provider-rfc="' +
      jsonFilesData[index].emisor.rfcEmisor +
      '" data-serie="' +
      ...
      '<select class="select-payform browser-default" data-file-index="' +
      index +
      '" onchange="changeDestiny(this);">' +
...
function setPayform(index) {
  if (
    jsonFilesData[index].comprobante.formaPago == "01" ||
    jsonFilesData[index].comprobante.formaPago == "02" ||
    jsonFilesData[index].comprobante.formaPago == "03"
  ) {
    $('.select-payform[data-file-index="' + index + '"]').val(
      jsonFilesData[index].comprobante.formaPago
    );
  }
...
function changeDestiny(select) {
  ...
  if ($(select).val() == 1) {
    ...
    $(".destiny-" + index_row).html("");
    $(".destiny-" + index_row).append(
      '<div class="input-field">' +
        '<input id="check-number-' +
        index_row +
        '" type="text" class="validate check" pattern="[0-9]+" data-length="5" maxlength="5">' +
        '<label for="check-number-' +
        index_row +
        '">Número de cheque</label>' +
        "</div>"
    );
...
function setConceptsToJson(json_index) {
  ...
  $("#conceptList" + json_index + " li div div:nth-child(2) select").each(
    function () {
      counterpart_list.push(
        $(this).find(":selected").attr("data-accounting-account-number")
      );
    }
  );
...
function verifyProvider(row_index) {
  ...
  $.ajax({
    ...
    data: {
      _token: CSRF_TOKEN,
      handler: "getProvider",
      provider_rfc: jsonFilesData[row_index].emisor.rfcEmisor,
    },
  })
    .done(function (data) {
      if (data.length === 0) {
        $(
          'tr[data-file-index="' +
            row_index +
            '"] td:nth-child(4) span.truncate'
        ).addClass("red-text");
      } else {
        $(
          'tr[data-file-index="' +
            row_index +
            '"] td:nth-child(7) .newProviderFromProvision'
        ).remove();
        $("#modalShowConcepts" + row_index + " .accounting-account-list").each(
          function () {
...
function setProviderAccountToJson(row_index, provider_accounting_account) {
  $.extend(
    jsonFilesData[row_index].proveedor.cuentaContable,
    provider_accounting_account
  );
}
...
function sendJsonFiles() {
  ...
  if (verifyUnregisteredProviders() != 0) {
    Materialize.toast(
      "No se pudo procesar la petición. El proveedor de algún comprobante no está registrado.",
      4000
    );
  } else {
    if (verifyUndefinedDates() != 0) {
      Materialize.toast(
        "No se pudo procesar la petición. Uno o varios XML no tienen fecha definida.",
        4000
      );
    } else if (verifyUndefindedOriginAccounts() != 0) {
      Materialize.toast(
        "No se pudo procesar la petición. Hay cuentas de origen no seleccionadas.",
        4000
      );
    } else if (verifyUndefindedChecks() != 0) {
      Materialize.toast(
        "No se pudo procesar la petición. Hay números de cheque no definidos.",
        4000
      );
    }
    ...
    // }
      ...
      $("tbody tr").each(function (index) {
        ...
        jsonFilesData[indexJsonFile].comprobante.fecha = $(
          "#fecha-" + indexJsonFile
        ).val();

        if (bank_accounts_lenght == 2) {
          jsonFilesData[indexJsonFile].datosOrigen.cuentaBancariaOrigen = $(
            ".select-bank-account optgroup option"
          ).attr("data-bank-account-number");
          jsonFilesData[indexJsonFile].datosOrigen.cuentaContableOrigen = $(
            ".select-bank-account optgroup option"
          ).val();
          jsonFilesData[indexJsonFile].datosOrigen.bancoOrigen = $(
            ".select-bank-account optgroup option"
          )
            .parent()
            .attr("data-bank-sat-key");
        } else {
          jsonFilesData[indexJsonFile].datosOrigen.cuentaBancariaOrigen = $(
            ".origin-" + indexJsonFile + " select option:selected"
          ).attr("data-bank-account-number");
          jsonFilesData[indexJsonFile].datosOrigen.cuentaContableOrigen = $(
            ".origin-" + indexJsonFile + " select option:selected"
          ).val();
        ...
        jsonFilesData[indexJsonFile].comprobante.formaPago = $(
          '.select-payform[data-file-index="' +
            indexJsonFile +
            '"] option:selected'
        ).val();
        jsonFilesData[indexJsonFile].comprobante.total = $(
          "#total-" + indexJsonFile
        ).val();
        ...
        if (
          ...
          jsonFilesData[indexJsonFile].datosDestino.numeroCheque = $(
            "#check-number-" + indexJsonFile
          ).val();
      ...
      $.ajax({
        ...
        data: {
          ...
          jsonFiles: jsonFiles,
          generateByProvider: generate_by_toggle,
          cfdiIndexSerie: $("#cfdi_index_serie").val(),
        },
      })
        .done(function (data) {
